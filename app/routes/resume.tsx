    ...
    return (
        <main className="bg-gradient-to-br from-slate-50 via-white to-indigo-50">
            ...
            <section className="main-section">
                <Link to="/" className="back-button w-fit hover:bg-slate-50 transition-all duration-200">
                ...
                <div className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)]">
                    <aside className="space-y-6">
                        <div className="rounded-3xl border border-slate-100 bg-white/95 p-6 shadow-xl shadow-slate-200/50 backdrop-blur">
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-bold text-slate-900">Resume preview</h2>
                                {resumeUrl && !isLoadingResume && !error && (
                                    <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="secondary-button text-sm transition-all duration-200 hover:scale-105">
                                        Open PDF →
                                    </a>
                                )}
                            </div>
                            <div className="mt-6">
                                {isLoadingResume && (
                                    <div className="flex h-[520px] items-center justify-center rounded-2xl border border-slate-100 bg-slate-50 text-sm text-slate-500">
                                ...
                                {!isLoadingResume && !error && imageUrl && (
                                    <img
                                        src={imageUrl}
                                        className="h-[520px] w-full rounded-2xl border-2 border-slate-100 object-contain bg-white shadow-inner animate-in fade-in duration-500"
                                        alt="Resume preview"
                                    />
                                )}
                                {!isLoadingResume && error && (
                                    <div className="flex h-[520px] items-center justify-center rounded-2xl border-2 border-red-200 bg-red-50 p-6 text-center text-sm text-red-700">
                                        <div className="flex flex-col items-center gap-3">
